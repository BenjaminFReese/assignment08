...
votes_data <- bind_cols(
  select(dataset), votes_kmeans_fit %>%
    extract_recipe() %>%
    bake(dataset),
  cluster = votes_kmeans_fit %>%
...
## Fitting Model
votes_kmeans_fit <- votes_kmeans_wflow %>%
  fit(data = votes_numeric)

## Creating Dataframe to visualize
votes_data <- bind_cols(
  select(votes_numeric), votes_kmeans_fit %>%
    extract_recipe() %>%
    bake(votes_numeric),
  cluster = votes_kmeans_fit %>%
    extract_cluster_assignment() %>%
